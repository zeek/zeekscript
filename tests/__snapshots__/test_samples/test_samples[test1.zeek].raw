...
const deltas2: vector of double = {
	0.01,
	0.02, # WHOA!
	0.01,
	0.03, # DOUBLE whoa!
...
function a_function(a: int, b: count, another_argument_for_linewrapping: string)
	...
	if ( a_long_var_a in a_long_var_b
	    && ( c in d || e in f )
...
@endif
	...
	local length = bytestring_to_count(data[mres$off - 1 + 8 : mres$off - 1 + 10 + 23 / 15
	    - 3]);
	...
	if ( |foo| > 0 )
		...
		print "Lovely patio around the fountain. "
		    ...
		    + "Ordered the Eutropia pizza thin crust-YUM! "
		    + "Will go back the next time I'm in Berkeley.";
